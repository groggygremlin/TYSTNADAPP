...
// ---- 16. Intro version footer renders ----
{
  ...
  assert(vn && vn.textContent === "v118", "41. Intro version footer shows the current release");
}
...
// ---- v22-A. VERSION constant reads v27 ----
{
  ...
  assert(SRC.includes('const VERSION = "v118"'), "166. app.js VERSION pin matches the current release");
}

// ---- v22-B. SW cache name is tystnad-v33 ----
{
  const SW = fs.readFileSync("sw.js", "utf8");
  assert(SW.includes('"tystnad-v118"'), "167. sw.js cache name is tystnad-v33");
}
...
{
  const SRC = fs.readFileSync("app.js", "utf8");
  assert(SRC.includes('const VERSION = "v118"'), "172. app.js VERSION pin matches the current release");
}

// ---- v23-D. SW cache name is tystnad-v33 ----
{
  const SW = fs.readFileSync("sw.js", "utf8");
  assert(SW.includes('"tystnad-v118"'), "173. sw.js cache name is tystnad-v33");
}
...
{
  const SW = fs.readFileSync("sw.js", "utf8");
  assert(SW.includes('"tystnad-v118"'), "190. sw.js cache tystnad-v33");
}

// ---- v27-B. VERSION v27 ----
{
  const SRC = fs.readFileSync("app.js", "utf8");
  assert(SRC.includes('const VERSION = "v118"'), "191. app.js VERSION pin matches the current release");
}
...
// ---- v28-A. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "224. sw.js cache tystnad-v118");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "225. app.js VERSION pin matches the current release");
}
...
// ---- v30-B. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "264. sw.js cache tystnad-v33");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "265. app.js VERSION pin matches the current release");
}
...
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "278. sw.js cache tystnad-v33 (v31 check)");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "279. app.js VERSION pin matches the current release");
}
...
// ---- v32-C. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "284. sw.js cache tystnad-v33");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "285. app.js VERSION pin matches the current release");
}
...
// ---- v33-D. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "290. sw.js cache tystnad-v33 (v33 check)");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "291. app.js VERSION pin matches the current release");
}
...
// ---- v34-I. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "314. sw.js cache tystnad-v36");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "315. app.js VERSION pin matches the current release");
}
...
// ---- v35-E. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "326. sw.js cache tystnad-v36");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "327. app.js VERSION pin matches the current release");
}
...
// ---- v36-F. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "341. sw.js cache tystnad-v37");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "342. app.js VERSION pin matches the current release");
}
...
// ---- v37-K. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "376. sw.js cache tystnad-v39");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "377. app.js VERSION pin matches the current release");
}
...
// ---- v40-A. sw.js + VERSION ----
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "398. sw.js cache tystnad-v41");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "399. app.js VERSION pin matches the current release");
}
...
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "452. sw.js cache tystnad-v45");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "453. app.js VERSION pin matches the current release");
}
...
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "478. sw.js cache tystnad-v118");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "479. app.js VERSION pin matches the current release");
}
...
{
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "492. app.js VERSION pin matches the current release");
  assert(fs.readFileSync("sw.js","utf8").includes('"tystnad-v118"'), "493. sw.js cache tystnad-v118");
}
...
{
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "528. app.js VERSION pin matches the current release");
  assert(fs.readFileSync("sw.js","utf8").includes('"tystnad-v118"'),          "529. sw.js cache tystnad-v118");
}
...
{
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "543a. app.js VERSION pin matches the current release");
  assert(fs.readFileSync("sw.js","utf8").includes('"tystnad-v118"'), "543b. sw.js cache tystnad-v118");
}
...
{
  assert(fs.readFileSync("sw.js","utf8").includes("tystnad-v118"), "563. sw.js cache tystnad-v118");
  assert(fs.readFileSync("app.js","utf8").includes('const VERSION = "v118"'), "564. app.js VERSION pin matches the current release");
}
...
// ---- v89. Privacy notice before joining a table (peer review finding 9) ----
{
  ...
  /* v112: the notice merged INTO the instruction, because the two said the same thing twice.
     It keeps its id, so the tripwire below still guards it. It is now at the very top rather
     than immediately above the button; the "consent at the moment of decision" argument that
     put it there expired when the page shrank to one viewport and both became visible at once. */
  assert(kids.indexOf(notice) < kids.indexOf(d.getElementById("tl-join-btn")),
    "1165. It still appears above the connect button");
  assert(/Ask your GM for the join code/.test(text),
    "1165a. And carries the instruction with it, rather than repeating what it already said");
...
const unlinkBehaviour = (async () => {
  ...
  /* Re-entering must not show a stale failure from last time. v112: the sign-out block moved
     to the INTRO, so renderIntro is what clears it now, not openTableLink. The guarantee is
     unchanged; only the screen that owns it moved. */
  {
    const { d, w } = linkedDOM(offline);
    await w.eval("tlDoUnlink()");
    assert(visible(d.getElementById("tl-forget-btn")), "1112. Hatch visible after the failure");
    w.eval("renderIntro();");
    assert(hidden(d.getElementById("tl-unlink-error")), "1113. Returning to the front door clears the stale error");
    assert(hidden(d.getElementById("tl-forget-btn")), "1114. And hides the stale hatch");
  }
})();
...
/* ---- v118: sign-out asks first ---- */
{
  const APP118 = fs.readFileSync("app.js", "utf8");

  /* v112 moved sign-out to the front door, which put a one-tap, connection-requiring action on
     the launch screen. The cost is not data, since the Explorer stays, but REACH: getting back
     in needs signal, which a player at a table may not have. Tomas ruled the confirm in. */
  const armed = () => makeDOM(WARRIOR, { "tystnad-tablelink": JSON.stringify({
    token: "T", ownsTableLink: true, email: "t@example.com" }) });

  {
    const { d, w } = armed();
    click(d.getElementById("tl-unlink-btn"));
    assert(visible(d.getElementById("overlay-confirm")), "1540. Sign-out asks before it acts");
    assert(w.localStorage.getItem("tystnad-tablelink") !== null,
      "1541. And has done NOTHING yet: the token is still there while he decides");
    assert(/Signing back in needs a connection/.test(d.getElementById("confirm-text").textContent),
      "1542. The prompt states the cost, which is reach rather than data");
    assert(d.getElementById("confirm-yes").textContent === "Sign out" &&
           d.getElementById("confirm-no").textContent === "Stay signed in",
      "1543. Both buttons name what they do, rather than saying yes and no");
  }

  // Backing out leaves him exactly as he was.
  {
    const { d, w } = armed();
    click(d.getElementById("tl-unlink-btn"));
    click(d.getElementById("confirm-no"));
    assert(hidden(d.getElementById("overlay-confirm")), "1544. Backing out closes the prompt");
    assert(w.localStorage.getItem("tystnad-tablelink") !== null, "1545. And leaves him signed in");
    assert(hidden(d.getElementById("screen-gate")), "1546. Still inside the app, not at the front door");
  }

  /* THE SHARED OVERLAY MUST NOT HAVE DRIFTED. Abandoning an Explorer is destructive and its
     wording is the only thing standing in front of it, so parameterising the prompt has to
     leave that path saying exactly what it always said. */
  {
    const { d } = makeDOM(WARRIOR);
    click(d.getElementById("btn-new-explorer"));
    assert(visible(d.getElementById("overlay-confirm")), "1547. Abandon still asks");
    assert(d.getElementById("confirm-text").textContent === "Abandon this Explorer? He will not return.",
      "1548. In exactly the words it used before the overlay was shared");
    assert(d.getElementById("confirm-yes").textContent === "Abandon" &&
           d.getElementById("confirm-no").textContent === "Keep Him",
      "1549. With its own buttons, not sign-out's");
  }

  // Two callers, one overlay, and the wording follows whoever opened it.
  {
    const { d } = armed();
    click(d.getElementById("tl-unlink-btn"));
    click(d.getElementById("confirm-no"));
    click(d.getElementById("btn-new-explorer"));
    assert(/Abandon this Explorer/.test(d.getElementById("confirm-text").textContent),
      "1550. A second caller reworded it, so no prompt is ever left wearing the last one's text");
  }

  assert(/function askConfirm\(text, yesLabel, noLabel, action\)/.test(APP118),
    "1551. One helper, so a third caller cannot hard-wire a fourth set of strings into the markup");
}

/* ---- v117: the Initiative block halved, on a ruled deviation from Law 2 ---- */
{
  const CSS117 = fs.readFileSync("style.css", "utf8");
  const rule = (sel) => (CSS117.match(new RegExp("(?:^|\\n)" + sel.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "[^{]*\\{([^}]*)\\}")) || ["",""])[1];

  /* Law 2 names `.vital-label` as a display element at 1.5rem. Tomas ruled it smaller HERE on
     2026-07-29 ("shrink the label to fit"), because since v106 this label sits inline beside
     its own value rather than heading a panel, and at 1.5rem the caps broke to three lines and
     held the block at 105px against the 73px he asked for. Recorded in CLAUDE.md so a later
     pass reads it as a ruling rather than as drift. */
  assert(/font-size:\s*1\.5rem/.test(rule(".vital-label")),
    "1535. Law 2 still holds for .vital-label itself, at 1.5rem");
  assert(/font-size:\s*1\.1rem/.test(rule(".init-block-row .vital-label")),
    "1536. The deviation is SCOPED to the Initiative block and nothing else");
  assert(/Law 2/.test(CSS117.slice(Math.max(0, CSS117.indexOf(".init-block-row .vital-label") - 900),
                                   CSS117.indexOf(".init-block-row .vital-label"))),
    "1537. And says in the stylesheet that it is a ruled exception, not an accident");

  // Only one element wears the class, so the deviation cannot leak by accident today.
  assert((HTML.match(/class="vital-label"/g) || []).length === 1,
    "1538. Exactly one .vital-label exists, and it is the one Tomas ruled on");

  // The record carries the exception too, so the law and its exception travel together.
  const CLAUDE = fs.readFileSync("CLAUDE.md", "utf8");
  assert(/init-block-row .vital-label|Initiative Roll Contribution/.test(CLAUDE),
    "1539. CLAUDE.md records the exception, so the law is not read as unbroken");
}

/* ---- v116: the last tap-target flag, and Rules Reference in bone ---- */
{
  const CSS116 = fs.readFileSync("style.css", "utf8");
  const rule = (sel) => (CSS116.match(new RegExp("(?:^|\\n)" + sel.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "[^{]*\\{([^}]*)\\}")) || ["",""])[1];

  /* `.hp-max` measured 31x34 against the 44px rule, and could NOT be widened in the flow: the
     HP strip has 2px of spare width at 375px, and widening this button by 13 would have put it
     back over the screen, which is the defect v105 fixed. So the HIT AREA grows by padding and
     equal negative margins cancel it, leaving every neighbour where it was. jsdom cannot
     measure geometry, so this pins the mechanism; the browser confirmed 44.4 x 45.5 with the
     readout unchanged at 72px and the full 6px gap to the HP + button. */
  {
    const hm = rule("button.hp-max");
    assert(/padding:\s*12px 4px 12px 17px/.test(hm), "1529. The hit area is grown with padding");
    assert(/margin:\s*-6px 0 -6px -13px/.test(hm), "1530. And equal negative margins keep the layout still");
    /* THE ARITHMETIC THAT WENT WRONG TWICE, pinned so it cannot go wrong again: each margin
       cancels only what its OWN padding ADDED, never the whole padding. The first attempt
       over-cancelled and pulled the readout from 72px to 64px, moving the HP + button with it,
       which is precisely what this technique exists to prevent. Original padding was 6px
       vertical and 4px horizontal, so: left 17 adds 13 and is cancelled by -13; right stays 4
       and is cancelled by 0; vertical 12 adds 6 each side and is cancelled by -6. */
    const pad = hm.match(/padding:\s*(\d+)px (\d+)px (\d+)px (\d+)px/).slice(1).map(Number);
    // `(?:px)?` because a zero-length margin is written `0`, with no unit.
    const mar = hm.match(/margin:\s*(-?\d+)(?:px)? (-?\d+)(?:px)? (-?\d+)(?:px)? (-?\d+)(?:px)?/).slice(1).map(Number);
    const ORIGINAL = [6, 4, 6, 4];
    const cancelsExactly = pad.every((v, i) => v - ORIGINAL[i] === -mar[i]);
    assert(cancelsExactly,
      "1531. Every margin cancels exactly what its own padding added, so no neighbour moves");
    assert(pad[3] > pad[1],
      "1532. The extra width goes LEFT over the HP number, which is text, not right toward the + button");
  }

  // Rules Reference heads in bone, matching what v114 did for the Expedition guide.
  assert(/#handbook-body \.rules-topic-title \{[^}]*color:\s*var\(--bone\)/.test(CSS116),
    "1533. Rules Reference topic heads are bone");
  assert(!/#handbook-body \.howto-h \{[^}]*color/.test(CSS116),
    "1534. And the scrolling section heads keep blood-bright, which Tomas did not ask to change");
}

/* ---- v114: Handbook and Expedition headers ---- */
{
  const CSS114 = fs.readFileSync("style.css", "utf8");
  const rule = (sel) => (CSS114.match(new RegExp("(?:^|\\n)" + sel.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "[^{]*\\{([^}]*)\\}")) || ["",""])[1];

  // Handbook: both header kinds in caps, colour untouched.
  const hb = CSS114.match(/#handbook-body \.howto-h,\s*\n#handbook-body \.rules-topic-title \{([^}]*)\}/);
  assert(hb && /text-transform:\s*uppercase/.test(hb[1]),
    "1512. Handbook section headers and Rules Reference topics are both capitalised");

  // Expedition: bone AND caps.
  const ex = rule("#expedition-guide .rules-topic-title");
  assert(/text-transform:\s*uppercase/.test(ex), "1513. Expedition section headers are capitalised");
  assert(/color:\s*var\(--bone\)/.test(ex), "1514. And bone rather than blood");

  /* Scoping is the whole point, and it is what an unscoped rule would have broken.
     .rules-topic-title is shared by three surfaces; Tomas asked for two of them. */
  assert(!/(?:^|\n)\.rules-topic-title \{[^}]*text-transform/.test(CSS114),
    "1515. The shared class is NOT capitalised globally");
  assert(!/#action-card \.rules-topic-title \{[^}]*text-transform/.test(CSS114),
    "1516. So the Combat action card, which Tomas did not ask about, is untouched");

  // The block heading above the Expedition accordion stays as it was, by his ruling.
  assert(/<p class="field-label">Hexploration<\/p>/.test(HTML),
    "1517. HEXPLORATION keeps its own treatment and is not restyled with the sections beneath it");
}

/* ---- v112: the account moves to the front door ---- */
{
  const APP = fs.readFileSync("app.js", "utf8");
  /* Read through the RENDER, not through `tlDevice`: top-level `let` bindings are unreachable
     via w.eval, and what the player actually sees is the honest witness anyway. */
  /* v113: read the whole LINE, because the sentence itself branches now: an address gives
     "Signed in as x@y", and its absence gives "Signed in on this device" rather than the
     sentence completing itself with the word "device" where an account should be. */
  const shown = (blob) =>
    makeDOM(WARRIOR, { "tystnad-tablelink": JSON.stringify(blob) })
      .d.getElementById("intro-account-line").textContent;

  assert(shown({ token: "T", ownsTableLink: true, email: "player@example.com" }) === "Signed in as player@example.com",
    "1497. The signed-in address is stored, loaded and named");
  assert(!shown({ token: "T", ownsTableLink: true, email: "x".repeat(9000) }).includes("xxxx"),
    "1498. An absurd address is refused rather than rendered");
  assert(shown({ token: "T", ownsTableLink: true, email: { not: "a string" } }) === "Signed in on this device",
    "1499. A hand-edited one of the wrong type falls back to the honest sentence");

  assert(/function gateAcceptToken\(r, email\)/.test(APP),
    "1500. ONE choke point, so no path can mint a token without recording an owner");
  /* Lookbehind, because "gateAcceptToken(r, email)" also matches the FUNCTION DEFINITION.
     Without it this passed while the sign-in path had stopped passing the address at all,
     which is the false-pass half of the substring trap and the dangerous half. */
  assert(/(?<!function )gateAcceptToken\(r, email\)/.test(APP),
    "1501. The SIGN-IN CALL records the address it signed in with");
  assert(/gateAcceptToken\(r, gatePendingEmail\)/.test(APP),
    "1502. And the emailed-code path does too, covering registration and reset");

  {
    const { d } = makeDOM(WARRIOR, { "tystnad-tablelink": JSON.stringify({
      token: "T", ownsTableLink: true, email: "tomas@example.com" }) });
    assert(visible(d.getElementById("intro-account")), "1503. The account block is on the front door");
    assert(d.getElementById("intro-account").contains(d.getElementById("tl-unlink-btn")),
      "1504. With the way out beside it, where an account action belongs");
    const kids = [...d.querySelector(".intro-layout").children];
    assert(kids.indexOf(d.getElementById("intro-account")) < kids.indexOf(d.getElementById("intro-version-note")),
      "1505. Above the version number, which is where Tomas asked for it");
    /* v113: `margin-top: auto` alone collapses to zero once the intro fills the viewport,
       which is how the block ended up hard against How to Play on a shorter Android screen.
       The padding is the floor that auto cannot provide. */
    const CSSv113 = fs.readFileSync("style.css", "utf8");
    const acctRule = (CSSv113.match(/(?:^|\n)\.intro-account \{([^}]*)\}/) || ["",""])[1];
    assert(/margin-top:\s*auto/.test(acctRule) && /padding-top:\s*26px/.test(acctRule),
      "1505a. And never closer than 26px to the buttons above, however short the screen");
    assert(/\.intro-account \.tl-forget-note \{[^}]*text-align:\s*center/.test(CSSv113),
      "1505b. The sign-out caveat is centred like everything else in the block");
  }

  {
    const { d } = makeDOM(WARRIOR, { "tystnad-tablelink": JSON.stringify({ token: "T", ownsTableLink: true }) });
    assert(visible(d.getElementById("intro-account")), "1506. A player upgrading into v112 still gets the block");
    /* v113: the whole sentence changes. "Signed in as this device" read as though the device
       were the account, which is what Tomas saw on his Android after upgrading. */
    assert(d.getElementById("intro-account-line").textContent === "Signed in on this device",
      "1507. A player who registered before v112 gets an honest sentence, not a gap and not a device posing as an account");
    assert(!/Signed in as/.test(d.getElementById("intro-account-line").textContent),
      "1507a. And is never told he is signed in AS a device");
  }

  {
    const { d } = makeDOM(WARRIOR);
    click(d.getElementById("btn-continue"));
    assert(!d.getElementById("tl-state-lobby").contains(d.getElementById("tl-unlink-btn")),
      "1508. Sign-out is gone from Table Link, where it had been stranded since v99");
    assert(/id="tl-open-site"[^>]*>View My Account</.test(HTML), "1509. The account link is renamed");
    const lobby = [...d.getElementById("tl-state-lobby").children];
    assert(lobby.indexOf(d.getElementById("tl-open-site")) === lobby.length - 1,
      "1510. And sits at the foot of the screen");
    const row = d.querySelector(".tl-field-row");
    assert(row && row.contains(d.getElementById("tl-join-code")) && row.contains(d.getElementById("tl-display-name")),
      "1511. Join code and display name share a row");
  }
}
...
/* ---- v115, APP-009: the device label reaches the server on sign-in ----

   The route accepted an optional deviceLabel on /app/{verify,login,reset}; the app sent it on
   the emailed-code path only, so every sign-in arrived in the GM site's Office as a blank row.
   Raised by the site coder while diagnosing APP-008, where two unlabelled devices on two
   accounts turned a one-screenshot diagnosis into a two-screenshot one. */
const deviceLabelBehaviour = (async () => {
  const tick = () => new Promise((r) => setTimeout(r, 0));
  const signIn = async (edit) => {
    const dom = new JSDOM(HTML, { url: "http://localhost/", runScripts: "outside-only" });
    const w = dom.window;
    await tick();                       // let jsdom's own DOMContentLoaded pass first
    w.eval(`window.__sent = [];
      window.fetch = (u, o) => { window.__sent.push({ url: String(u), body: JSON.parse(o.body) });
        return Promise.resolve({ ok: true, status: 200,
          json: () => Promise.resolve({ deviceToken: "TOK", ownsTableLink: false }) }); };`);
    w.eval(APPJS);
    w.document.dispatchEvent(new w.Event("DOMContentLoaded"));
    const d = w.document;
    click(d.getElementById("gate-to-signin"));
    const label = d.getElementById("gate-si-label");
    const prefilled = label.value;
    if (edit !== undefined) label.value = edit;
    d.getElementById("gate-si-email").value = "player@example.com";
    d.getElementById("gate-si-password").value = "a-long-enough-password";
    click(d.getElementById("gate-si-btn"));
    for (let i = 0; i < 4; i++) await tick();
    return { prefilled, sent: w.__sent[0], d };
  };

  {
    const { prefilled, sent, d } = await signIn();
    assert(d.getElementById("gate-si-label"), "1518. Sign-in has a device name field, which it never had before v115");
    assert(prefilled.length > 0,
      "1519. PREFILLED, because an OPTIONAL field left blank produces exactly the blank row the finding is about");
    assert(d.getElementById("gate-si-label").getAttribute("maxlength") === "100",
      "1520. Bounded to what the route accepts");
    assert(/\/api\/v1\/app\/login$/.test(sent.url), "1521. Sign-in still posts to login");
    assert(sent.body.deviceLabel === prefilled,
      "1522. AND CARRIES THE LABEL. What is shown in the field is what reaches the server");
  }

  // A name he types is his, not the suggestion.
  {
    const { sent } = await signIn("Table tablet");
    assert(sent.body.deviceLabel === "Table tablet", "1523. A typed name overrides the suggestion");
  }

  // Cleared deliberately means no key at all, rather than an empty string in the row.
  {
    const { sent } = await signIn("");
    assert(!("deviceLabel" in sent.body),
      "1524. Clearing it sends no key, rather than an empty label the server would store");
  }

  // The suggestion is coarse and derived, not a fingerprint.
  {
    const src = fnBody(APPJS, "gateDeviceName");
    assert(/iPad/.test(src) && /iPhone/.test(src) && /Android/.test(src),
      "1525. The suggestion names a handful of device kinds");
    assert(/maxTouchPoints/.test(src),
      "1526. And tells an iPad from a Mac, which iPadOS makes necessary and which is the device that exposed APP-009");
    assert(!/canvas|WebGL|fonts|screen\.width|hardwareConcurrency/i.test(src),
      "1527. Derived from the user agent alone, so it is a label and not a fingerprint");
  }

  // A name he typed survives leaving the state and coming back.
  {
    const dom = new JSDOM(HTML, { url: "http://localhost/", runScripts: "outside-only" });
    const w = dom.window;
    await tick();
    ...
    const d = w.document;
    click(d.getElementById("gate-to-signin"));
    d.getElementById("gate-si-label").value = "Kitchen iPad";
    click(d.getElementById("gate-to-register"));
    click(d.getElementById("gate-to-signin"));
    assert(d.getElementById("gate-si-label").value === "Kitchen iPad",
      "1528. It only ever fills a field he left empty, so a name he typed is not overwritten");
  }
})();

Promise.all([swBehaviour, unlinkBehaviour, sessionSurvivalBehaviour, updateCheckBehaviour,
             reloadResumeBehaviour, gateBehaviour, deviceLabelBehaviour]).then(finishRun, (e) => {
